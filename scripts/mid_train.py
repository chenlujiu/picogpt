...
from picogpt.common import compute_init, compute_cleanup, print0, DummyWandb, get_base_dir, autodetect_device_type
from picogpt.tokenizer import get_token_bytes
from picogpt.checkpoint_manager import save_checkpoint
...
def mid_data_generator(split):
    ...
    while True:
        ...
        targets = targets_cpu.view(device_batch_size, max_seq_len).to(device=device, dtype=torch.int64,
                                                                      non_blocking=True)
        if split == "train":
            if num_iterations > 0:
